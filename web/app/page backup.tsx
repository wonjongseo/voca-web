...
export default function Home() {
  ...
  return <div className="app-shell">
    ...
    <main><section className="page-heading"><div><p className="eyebrow">{page==='words'?'WORDS THAT STAY WITH YOU':page==='study'?'A LITTLE PRACTICE, EVERY DAY':page==='wrong'?'WORDS TO MEET AGAIN':'YOUR GROWTH, ONE WORD AT A TIME'}</p><h1>{page==='words'?'나의 단어장':page==='study'?'오늘의 학습':page==='wrong'?'오답노트':'학습 기록'}</h1><p>{page==='words'?'발견한 단어를 모아, 나만의 언어로 만들어보세요.':page==='study'?'한 번 더 떠올리는 순간, 단어가 오래 남아요.':page==='wrong'?'헷갈렸던 단어를 다시 만나, 확실하게 기억해보세요.':'작은 반복이 쌓여, 더 넓은 어휘가 됩니다.'}</p></div>{page==='words'&&<button className="button primary" onClick={()=>setEditor(blankWord())} disabled={!ready||!!storageError}><Plus size={18}/>단어 추가</button>}</section>
    ...
    {page==='wrong'&&<section className="wrong-section">
      <div className="section-title wrong-section-title">
        <div>
          <h2>다시 볼 단어 <span>{wrongStats.length}</span></h2>
          <p className="muted">가장 최근 학습에서 틀린 단어만 모았어요. 다시 맞히면 자동으로 오답노트에서 빠집니다.</p>
        </div>
        <button className="button primary" disabled={!wrongWords.length} onClick={()=>{setScope('wrong');setPage('study');setQuiz(null);}}>
          <RotateCcw size={17}/>오답 단어 학습하기
        </button>
      </div>

      {wrongStats.length?<div className="wrong-list">
        {wrongStats.map(({word,wrong,correct,total,accuracy,lastWrong})=><article className="wrong-card" key={word.id}>
          <div className="wrong-card-head">
            ...
            <span className="wrong-badge"><RotateCcw size={13}/>현재 오답</span>
          </div>

          <div className="wrong-metrics">
            <div><span>오답</span><strong className="wrong-value">{wrong}<small>회</small></strong></div>
            <div><span>정답</span><strong>{correct}<small>회</small></strong></div>
            <div><span>정답률</span><strong>{accuracy}<small>%</small></strong></div>
            <div><span>최근 오답</span><strong className="wrong-date">{lastWrong||'-'}</strong></div>
          </div>

          <div className="wrong-accuracy">
            <div className="wrong-accuracy-head"><span>누적 정답률</span><span>{accuracy}%</span></div>
            <div className="wrong-progress"><div style={{width:`${accuracy}%`}}/></div>
          </div>
        ...
        <Check size={34}/><h3>현재 오답이 없어요.</h3>
        <p>틀렸던 단어도 다시 맞히면 여기서 자동으로 사라져요.</p>
