...
type Session = {mode: Mode; words: Word[]; index: number; correct: number; choices: string[][]};
...
export default function Home() {
  ...
  function startQuiz(mode:Mode){let pool=scope==='due'?due:scope==='favorite'?db.words.filter(w=>w.favorite):db.words;if(scope==='due'&&!pool.length&&db.words.length){pool=db.words;setScope('all');setNotice('오늘 복습을 완료해서 모든 단어로 다시 학습합니다.');}if(!pool.length){setNotice(scope==='favorite'?'즐겨찾기한 단어가 없습니다.':'저장한 단어가 없습니다.');return;}const limit=quizSize==='all'?pool.length:Number(quizSize);const words=shuffle(pool).slice(0,limit);if(mode==='choice'&&new Set(db.words.map(w=>w.meaning)).size<2){setNotice('객관식 퀴즈에는 서로 다른 의미의 단어가 2개 이상 필요합니다.');return;}setQuiz({mode,words,index:0,correct:0,choices:words.map(w=>shuffle([w.meaning,...shuffle([...new Set(db.words.filter(d=>d.meaning!==w.meaning).map(d=>d.meaning))]).slice(0,3)]))});setAnswer('');setAcceptedTypo(false);setRevealed(false);setGraded(null);busyGrade.current=false;}
  function grade(correct:boolean){if(!quiz||graded!==null||busyGrade.current)return;busyGrade.current=true;const word=quiz.words[quiz.index];const current=dbRef.current;const latest=current.words.find(w=>w.id===word.id);if(!latest){setQuiz(null);busyGrade.current=false;return;}if(commit({...current,words:current.words.map(w=>w.id===word.id?schedule(w,correct):w),reviews:[...current.reviews,{date:localDate(),correct,wordId:word.id}]})){setGraded(correct);setRevealed(true);setQuiz({...quiz,correct:quiz.correct+(correct?1:0)});}else busyGrade.current=false;}
  ...
  return <div className="app-shell">
    ...
    {page==='study'&&<section className="study-section">{!quiz?<><div className="section-title"><h2>어떻게 학습할까요?</h2><div className="study-options"><label>학습 범위<select aria-label="학습 범위" value={scope} onChange={e=>setScope(e.target.value)}><option value="due">오늘 복습할 단어 ({due.length})</option><option value="all">모든 단어 ({db.words.length})</option><option value="favorite">즐겨찾기 ({db.words.filter(w=>w.favorite).length})</option></select></label><label>문제 수<select aria-label="문제 수" value={quizSize} onChange={e=>setQuizSize(e.target.value)}><option value="5">5개</option><option value="10">10개</option><option value="20">20개</option><option value="30">30개</option><option value="all">전체</option></select></label></div></div><div className="mode-grid">{modes.map(({id,name,desc,icon:Icon})=><button key={id} className="mode-card" onClick={()=>startQuiz(id)} disabled={!ready||!!storageError}><span className="mode-icon"><Icon size={28}/></span><h3>{name}</h3><p>{desc}</p><span className="mode-start">학습 시작<ArrowRight size={18}/></span></button>)}</div><div className="study-note"><Sprout size={22}/><span>오늘 {today.length}번 복습했어요. 작은 반복을 이어가세요.</span></div></>:!current?<div className="quiz-result"><span className="result-icon"><Check size={38}/></span><p className="eyebrow">SESSION COMPLETE</p><h2>오늘도 한 걸음 자랐어요.</h2><p>{quiz.words.length}개 중 {quiz.correct}개를 기억했어요.</p><strong>{Math.round(quiz.correct/quiz.words.length*100)}<small>%</small></strong><button className="button primary" onClick={()=>setQuiz(null)}>학습 목록으로<ArrowRight size={17}/></button></div>:<div className="quiz-wrap"><div className="quiz-top"><button className="button text-button" onClick={()=>setQuiz(null)}><ArrowLeft size={17}/>학습 종료</button><span>{modes.find(m=>m.id===quiz.mode)?.name} · {quiz.index+1} / {quiz.words.length}</span></div><div className="progress-track"><div style={{width:`${quiz.index/quiz.words.length*100}%`}}/></div><div className="question-area"><p className="eyebrow">{quiz.mode==='typing'?'이 의미의 영어 단어는?':'이 단어의 의미는?'}</p><h2>{quiz.mode==='typing'?current.meaning:current.word}</h2>{quiz.mode!=='typing'&&<button className="icon-button" aria-label="발음 듣기" title="발음 듣기" onClick={()=>speak(current.word)}><AudioLines size={23}/></button>}{quiz.mode==='flash'&&(revealed?<div className="revealed-answer"><MeaningList word={current}/><ExampleList word={current}/></div>:<button className="button" onClick={()=>setRevealed(true)}><RotateCcw size={17}/>정답 보기</button>)}{quiz.mode==='choice'&&<div className="choices">{quiz.choices[quiz.index].map((choice,i)=><button key={choice} className={`choice ${graded!==null&&choice===current.meaning?'correct':''} ${graded===false&&choice===answer?'incorrect':''}`} disabled={graded!==null} onClick={()=>{setAnswer(choice);grade(choice===current.meaning);}}><span>{i+1}</span>{choice}</button>)}</div>}{quiz.mode==='typing'&&<form className="typing-form" onSubmit={e=>{e.preventDefault();if(answer.trim()){const exact=answer.trim().toLowerCase()===current.word.trim().toLowerCase();const accepted=acceptsSpelling(answer,current.word);setAcceptedTypo(accepted&&!exact);grade(accepted);}}}><input ref={typingInputRef} aria-label="영어 단어 정답" autoComplete="off" autoCapitalize="none" spellCheck={false} placeholder="영어 단어를 입력하세요" value={answer} disabled={graded!==null} onChange={e=>setAnswer(e.target.value)}/><button className="button primary" disabled={!answer.trim()||graded!==null}>정답 확인</button></form>}{graded!==null?<div className={`feedback ${graded?'positive':'negative'}`}><strong>{graded?(acceptedTypo?'작은 오타가 있지만 정답이에요!':'잘 기억했어요!'):'다음에 한 번 더 만나봐요.'}</strong><p>정답: {current.word} · {current.meaning}</p><button className="button primary" onClick={nextQuestion}>{quiz.index+1===quiz.words.length?'결과 보기':'다음 단어'}<ArrowRight size={17}/></button></div>:quiz.mode==='flash'&&revealed&&<div className="self-grade"><button className="button" onClick={()=>grade(false)}><RotateCcw size={17}/>다시 볼게요</button><button className="button primary" onClick={()=>grade(true)}><Check size={17}/>기억했어요</button></div>}</div></div>}</section>}
